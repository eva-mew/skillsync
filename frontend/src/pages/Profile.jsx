...
const Profile = () => {
  ...
  const toggleSkill     = (s) => setForm(prev => ({ ...prev, skills:    prev.skills.includes(s)    ? prev.skills.filter(x => x !== s)    : [...prev.skills, s] }));
  const toggleInterest  = (i) => setForm(prev => ({ ...prev, interests: prev.interests.includes(i) ? prev.interests.filter(x => x !== i) : [...prev.interests, i] }));
  ...
  const type = form.onboardingType; // 'job' | 'startup' | 'both'
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '28px 24px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '24px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>👤 My Profile</h1>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)' }}>Update your skills and preferences to get better matches</p>
        </div>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: '8px' }}>
              <span className="badge badge-green">
                {type === 'job' ? '💼 Job Seeker' : type === 'startup' ? '💡 Startup Builder' : '🚀 Both'}
              </span>
            </div>
          </div>
        ...
        {/* Technical Skills — job & both only */}
        {type !== 'startup' && (
          <div className="card" style={{ padding: '24px', marginBottom: '16px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '16px' }}>
              🧠 Technical Skills
              <span style={{ fontSize: '13px', fontWeight: '400', color: 'var(--text-muted)', marginLeft: '8px' }}>({form.skills.length} selected)</span>
            </h3>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {allSkills.map(skill => (
                <button key={skill} onClick={() => toggleSkill(skill)} style={{
                  ...
                  fontSize: '13px', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: '500'
                }}>
        ...
        {/* Experience Level — job & both only */}
        {type !== 'startup' && (
          <div className="card" style={{ padding: '24px', marginBottom: '16px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '16px' }}>🎯 Experience Level</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px' }}>
              {[
                ...
                <button key={opt.val} onClick={() => setForm({ ...form, experience: opt.val })} style={{
                  ...
                  textAlign: 'center', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif'
                }}>
        ...
        {/* Startup Interests — startup & both only */}
        {type !== 'job' && (
          <div className="card" style={{ padding: '24px', marginBottom: '16px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '16px' }}>
              💡 Startup Interests
              <span style={{ fontSize: '13px', fontWeight: '400', color: 'var(--text-muted)', marginLeft: '8px' }}>({form.interests.length} selected)</span>
            </h3>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <div className="card" style={{ padding: '24px', marginBottom: '24px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '16px' }}>⚙️ Preferences</h3>
          <div style={{ display: 'grid', gridTemplateColumns: type === 'both' ? '1fr 1fr' : '1fr', gap: '20px' }}>

            {/* Work Preference — job & both only */}
            {type !== 'startup' && (
              <div>
                <label style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', display: 'block', marginBottom: '10px' }}>Work Preference</label>
            ...
            {/* Startup Budget — startup & both only */}
            {type !== 'job' && (
              <div>
                <label style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', display: 'block', marginBottom: '10px' }}>Startup Budget</label>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  {[
